...
public class PlayerShip {
    ...
    public PlayerShip() {
        ...
        model = OBJLoader.loadResource("spacecore/Sample.obj");
    }
    ...
    public Vector3f getPlaneIntersect(Vector3f vf, Vector3f vi) {
        Vector3f LineDir = new Vector3f(vf).sub(vi).normalize();

        Vector3f PlaneNormal = new Vector3f(0, 1, 0);
        Vector3f neg_Vi = vi.negate(new Vector3f());

        float d = neg_Vi.dot(PlaneNormal) / LineDir.dot(PlaneNormal);
        final Vector3f pt = LineDir.mul(d, new Vector3f()).add(vi);
    ...
    public void GetCameraVectors(Vector3f CameraPos, Vector3f CameraTarget, Vector3f CameraUp) {
        ...
        CameraPos.set(Position);
        CameraTarget.set(Forward).add(Position);
        CameraUp.set(Up);
    }
